...
import { PawPrint, Sun, Moon, CheckCircle2, Circle, RefreshCw, Loader2 } from 'lucide-react';
...
export default function PlayTracker() {
  ...
  const fetchData = async () => {
    setLoading(true);
    // ดึงเฉพาะบ้านที่ start_date ≤ date ≤ end_date (กำลังเข้าพักอยู่)
    const {data:bData} = await supabase
      .from('bookings').select('*')
      .lte('start_date', date).gte('end_date', date)
      .order('room_type');

    if(!bData?.length){setBookings([]); setPlayMap({}); setLoading(false); return;}
    const ids = bData.map(b=>b.id);

    // ดึง ops เพื่อกรองบ้านที่เช็คเอ้าท์แล้วออก
    const {data:opsData} = await supabase
      .from('booking_ops').select('booking_id,checked_in,checked_out').in('booking_id',ids);
    const opsMap = {};
    (opsData||[]).forEach(o=>{ opsMap[o.booking_id]=o; });

    // กรองเฉพาะบ้านที่ checked_in = true และ checked_out ≠ true
    const activeBookings = bData.filter(b => {
      const ops = opsMap[b.id];
      if(!ops) return false;           // ยังไม่เช็คอิน → ไม่แสดงในหน้าปล่อยเล่น
      if(ops.checked_out) return false; // เช็คเอ้าท์แล้ว → ซ่อน
      return ops.checked_in;           // เช็คอินแล้วและยังอยู่ → แสดง
    });

    setBookings(activeBookings);
    if(!activeBookings.length){setPlayMap({}); setLoading(false); return;}
    ...
    <div className="space-y-6 py-2">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/40 text-xs mt-0.5">แสดงเฉพาะบ้านที่เช็คอินแล้ว และยังไม่เช็คเอ้าท์</p>
            </div>
          </div>
      ...
      {loading?(
        ...
        <div className="text-center py-20 bg-white rounded-[2rem] border border-[#efebe9]">
          ...
          <p className="text-[#A1887F] font-bold text-sm">ไม่มีแมวที่เช็คอินแล้วและยังอยู่ในวันที่เลือก</p>
              ...
              return(
                <div key={b.id} className="grid grid-cols-[1fr_90px_90px] items-center px-5 py-3.5 hover:bg-[#FDFBFA] transition-colors">
                  <div className="min-w-0 pr-3">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black"
                        ...
                        <span className="w-1 h-1 rounded-full" style={{background:rc}}/>{b.room_type}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <button onClick={()=>togglePlay(b.id,b.cat_names,'morning')} disabled={saving[`${b.id}_morning`]}
                      ...
                      {saving[`${b.id}_morning`]?<Loader2 size={14} className="animate-spin text-amber-500"/>:mp?<CheckCircle2 size={16} className="text-white"/>:<Circle size={16} className="text-[#C4A99A]"/>}
                    </button>
                    {mp&&(
                      <div className="text-center">
                        <p className="text-[9px] font-black text-amber-600">{mp.released_at}</p>
                        {mp.returned_at?<p className="text-[9px] font-black text-green-600">↩ {mp.returned_at}</p>:<button onClick={()=>returnCat(mp.id)} className="text-[9px] font-black text-green-600 hover:underline">กลับแล้ว?</button>}
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <button onClick={()=>togglePlay(b.id,b.cat_names,'evening')} disabled={saving[`${b.id}_evening`]}
                      ...
                      {saving[`${b.id}_evening`]?<Loader2 size={14} className="animate-spin text-indigo-500"/>:ep?<CheckCircle2 size={16} className="text-white"/>:<Circle size={16} className="text-[#C4A99A]"/>}
                    </button>
                    {ep&&(
                      <div className="text-center">
                        <p className="text-[9px] font-black text-indigo-600">{ep.released_at}</p>
                        {ep.returned_at?<p className="text-[9px] font-black text-green-600">↩ {ep.returned_at}</p>:<button onClick={()=>returnCat(ep.id)} className="text-[9px] font-black text-green-600 hover:underline">กลับแล้ว?</button>}
                      </div>
                    )}
                  </div>
                </div>
